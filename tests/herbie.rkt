...
(define-const zero Math (Num (rational 0 1)))
(define-const one  Math (Num (rational 1 1)))
(define-const two  Math (Num (rational 2 1)))
(define-const three Math (Num (rational 3 1)))
(define-const neg-one Math (Neg (one)))
...
(define MinRational (semilattice 'MinRational Rational +inf.0 min))
(define MaxRational (semilattice 'MaxRational Rational -inf.0 max))
(function (hi Math) MinRational)
(function (lo Math) MaxRational)
...
(define-const sqrt5 Math (Sqrt (Num (rational 5 1))))
(define-const e14 Math
  (Div (one) (Sub (Div (Add (one) (sqrt5)) (two)) (Div (Sub (one) (sqrt5)) (two)))))
(define-const tgt Math (Div (one) (sqrt5)))
(run 6)
...
(check (= (e14) (tgt)))
(assert (= (print-size Add) 750))
(assert (= (print-size Sub) 630))
